...
from Utils.utils import fit_one_epoch, check_mkdir
...
#
# # assume tensor1 and tensor2 are your tensors
# tensor2_resized = F.interpolate(tensor2, size=(3000, 4096), mode='nearest')
# stacked_tensor = torch.stack([tensor1, tensor2_resized], dim=0)
def main(args):
    ...
    build_dataset(dataset_path=args.dataset_path,
    ...
    #   数据索引
    # ---------------------------------------------------------#
    with open(os.path.join(args.dataset_path, 'Data/train.txt'), "r") as f:
        train_lines = f.readlines()

    with open(os.path.join(args.dataset_path, 'Data/val.txt'), "r") as f:
        val_lines = f.readlines()

    # ---------------------------------------------------------#
    #   实例化数据集
    # ---------------------------------------------------------#
    train_dataset = MyDataset(dataset_path=args.dataset_path,
                              dataset_name=args.dataset_name,
    ...
    val_dataset = MyDataset(dataset_path=args.dataset_path,
                            dataset_name=args.dataset_name,
                            num_classes=args.num_classes,
                            dataset_trans=False,
                            transforms=get_transform(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path',        default='./Catch/bise/SMKNet_UAVID',   type=str, help='模型参数存放路径')
    ...
    parser.add_argument('--save_rate',     default=0.05, type=float, help='模型预测结果保存几率')
    parser.add_argument('--train_percent', default=0.6,  type=float, help='训练集')
    parser.add_argument('--val_percent',   default=0.2,  type=float, help='')
    ...
    parser.add_argument('--test_model',    default=False, type=bool, help='模型测试，仅加载少部分数据，设置为False即为正常训练')
    parser.add_argument('--pretrained',    default=False, type=bool, help='是否加载预训练参数')
    parser.add_argument('--model_path',    default='',    type=str, help='预训练参数存放地址')
